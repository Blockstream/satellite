...
you set up [in the beginning](#encryption-keys). With that, only you (the
...
message so that the recipient can verify it was really generated by you and not
altered in any way. To do so, use argument `--sign`. For example:
...
keyring, but you can specify the key as follows:
...
By default, API messages are broadcast worldwide through the six satellite
beams currently composing the Blockstream Satellite network. However, you can
also explicitly select the regions for each transmission.

For example, to send over regions 0 (G18) and 1 (E113) only, call the sender
